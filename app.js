/* =====================================================
   VB BUILT — FIELDSHEET  |  app.js  v3

   Matches the rebuilt index.html + style.css.
   Key changes from v2:
   - Job reference removed from fortnight details
     (captured per-day in the notes field instead)
   - Day type is a <select> dropdown, not pills
   - top-chrome height measured → margin-top set on content
   - Safe area handled in CSS; JS just measures chrome height
   ===================================================== */
...
/* ─────────────────────────────────────────────────────
   APP STATE
   All data the user enters lives here while they work.
───────────────────────────────────────────────────── */
let state = {
  employee: {
    name:          '',
    fortnightFrom: '',   // ISO "YYYY-MM-DD"
    fortnightTo:   ''    // Always 13 days after fortnightFrom
  },
...
/* Settings are stored separately so they persist across submissions */
let settings = {
  name:          '',
  emailTo:       '',
  emailCc:       '',
...
/* ─────────────────────────────────────────────────────
   ...
   Measures the fixed .top-chrome block and applies its
   height as margin-top on the scrollable content, so
   nothing is hidden behind the header.
   Re-measured on resize (e.g. orientation change).
───────────────────────────────────────────────────── */
      ...
      reg.addEventListener('updatefound', () => {
        ...
        nw.addEventListener('statechange', () => {
          if (nw.state === 'installed' && navigator.serviceWorker.controller) {
            ...
            // Chrome height may change; remeasure
...
function switchTab(btn) {
  ...
  /* Remove active/done from all step buttons */
...
/* ─────────────────────────────────────────────────────
   ...
   - Always snaps to the nearest Monday
...
function onFortnightStartChange(input) {
  ...
  /* End date = Monday + 13 days = Sunday */
...
function getNextFortnightStart(lastEndISO) {
  ...
  d.setDate(d.getDate() + 1); // Day after the Sunday = next Monday
  return toISO(d);
}

/* ─────────────────────────────────────────────────────
   BUILD DAILY TABLE
   Creates one row per day in the fortnight period.
   Each row has: day name | date | type dropdown | hours | notes
───────────────────────────────────────────────────── */
function buildDailyTable() {
  ...
  while (cur <= end) {
    ...
    /* Default type: weekday = work, weekend = rdo */
    const defaultType = isWeekend ? 'rdo' : 'work';
    ...
    /* Apply colour to the type dropdown immediately */
    const sel = row.querySelector('.day-select');
    applyTypeColour(sel, type);

    /* Highlight row if it already has hours */
  ...
  /* Re-measure top chrome as card appeared */
  setTimeout(setContentMargin, 50);
}
...
function onTypeChange(iso, sel) {
  ...
  /* Auto-fill 7.6 hrs for leave types if field is empty */
...
/* Sets the data-type attribute so CSS can colour the dropdown */
function applyTypeColour(sel, type) {
  sel.dataset.type = type;
}

/* Called when user types in the hours field */
...
function updateHoursSummary() {
  ...
  /* Build summary items — always show Total; others only if > 0 */
  const items = [
  ...
  el.innerHTML = items.map(i => `
    <div class="hrs-item">
      <span class="hrs-label">${i.label}</span>
      <span class="hrs-val ${i.cls}">${i.val.toFixed(1)}</span>
    </div>
  `).join('');
}
...
function copyToAllWeekdays() {
  ...
  if (!hrs || parseFloat(hrs) <= 0) {
    showToast('Enter hours first', 'error');
    return;
  }
  ...
  document.querySelectorAll('.day-row:not(.weekend)').forEach(row => {
    const iso = row.id.replace('row-', '');
    const hInput = document.getElementById('hours-' + iso);
    const tSel   = document.getElementById('type-'  + iso);
    if (hInput) {
      hInput.value = hrs;
...
/* ─────────────────────────────────────────────────────
   RECEIPT COMPRESSION
   Shrinks photos using the Canvas API before storing.
   Keeps receipt images under ~300 KB to speed up PDF.
───────────────────────────────────────────────────── */
function compressImage(file, maxWidth = 1200, quality = 0.75) {
  return new Promise((resolve, reject) => {
    /* PDFs and non-images: read as-is */
    ...
    img.onload = () => {
      ...
      resolve({ data, type: 'image/jpeg', originalSize: file.size, compressedSize });
    };
...
async function onReceiptSelected(input) {
  ...
  try {
    ...
    hint.textContent = saved > 5
      ? `${fmtBytes(r.originalSize)} → ${fmtBytes(r.compressedSize)} (saved ${saved}%)`
...
async function addExpense() {
  ...
  const btn = document.querySelector('#expense-form .btn--primary') ||
              document.querySelector('#tab-expenses .btn--primary');
  if (btn) { btn.textContent = '⏳ Processing…'; btn.disabled = true; }

  try {
    const compressed = await compressImage(file);
    state.expenses.push({
      id: Date.now(), type, amount: parseFloat(amount), date, desc,
...
function renderExpenses() {
  ...
  el.innerHTML = state.expenses.map((e,i) => `
    <div class="item-card">
      <div class="item-main">
        <div class="item-type">${escHtml(e.type)}</div>
        <div class="item-meta">${fmtDate(e.date)}${e.desc ? ' · '+escHtml(e.desc) : ''}</div>
        <div class="item-receipt">📎 ${escHtml(e.receiptName)}</div>
      </div>
      <div class="item-side">
        <div class="item-amount">$${e.amount.toFixed(2)}</div>
        <button class="item-remove" onclick="removeExpense(${i})">✕</button>
      </div>
    </div>
  `).join('');
}
...
function clearExpenseForm() {
  ...
  const lbl = document.getElementById('receipt-label');
  ...
  el.innerHTML = state.mileage.map((m,i) => `
    <div class="item-card">
      <div class="item-main">
        ...
        <div class="item-meta">${fmtDate(m.date)} · ${m.km} km @ $${m.rate.toFixed(2)}/km</div>
      </div>
      <div class="item-side">
        <div class="item-amount">$${m.total.toFixed(2)}</div>
        <button class="item-remove" onclick="removeMileage(${i})">✕</button>
      </div>
    </div>
  `).join('');
}
...
/* ─────────────────────────────────────────────────────
   COLLECT FORM DATA
   Reads every input into the state object.
───────────────────────────────────────────────────── */
...
function buildReview() {
  ...
  let html = `
    <div class="rv-section">
      ...
      <div class="rv-row"><span class="rv-key">Name</span><span class="rv-val">${escHtml(state.employee.name)||'—'}</span></div>
    ...
    <div class="rv-section">
      <div class="rv-title">Hours — ${totalHrs.toFixed(1)} total</div>
  ...
  if (state.expenses.length) html += `
    <div class="rv-section">
      <div class="rv-title">Expenses</div>
      ${state.expenses.map(e=>`
        <div class="rv-row">
          <span class="rv-key">${escHtml(e.type)}</span>
          <span class="rv-val">$${e.amount.toFixed(2)}</span>
        </div>`).join('')}
      <div class="rv-row"><span class="rv-key"><strong>Subtotal</strong></span><span class="rv-val"><strong>$${expTotal.toFixed(2)}</strong></span></div>
    </div>`;

  if (state.mileage.length) html += `
    <div class="rv-section">
      <div class="rv-title">Mileage</div>
      ${state.mileage.map(m=>`
        <div class="rv-row">
          <span class="rv-key">${escHtml(m.from)} → ${escHtml(m.to)}</span>
          <span class="rv-val">$${m.total.toFixed(2)}</span>
        </div>`).join('')}
      <div class="rv-row"><span class="rv-key"><strong>Subtotal</strong></span><span class="rv-val"><strong>$${milTotal.toFixed(2)}</strong></span></div>
    </div>`;

  if (state.allowances.length) html += `
    <div class="rv-section">
      <div class="rv-title">Allowances</div>
      ${state.allowances.map(a=>`
        <div class="rv-row">
          <span class="rv-key">${escHtml(a.type)}</span>
          <span class="rv-val">$${a.amount.toFixed(2)}</span>
        </div>`).join('')}
      <div class="rv-row"><span class="rv-key"><strong>Subtotal</strong></span><span class="rv-val"><strong>$${allTotal.toFixed(2)}</strong></span></div>
    </div>`;

  html += `
    <div class="rv-total">
      <span class="rv-total-label">💰 Grand Total</span>
      <span class="rv-total-amt">$${grandTotal.toFixed(2)}</span>
    </div>`;

  document.getElementById('review-content').innerHTML = html;

  /* Show/hide send methods */
...
async function sendViaEmailJS(pdfBlob, filename) {
  ...
  if (result.status !== 200) throw new Error('EmailJS status ' + result.status);
}
...
async function generatePDF() {
  ...
  const txt = (t, x, sz=10, style='normal', col=[15,31,53]) => {
  ...
  doc.text('Timesheet & Expense Submission  ·  Generated: '+new Date().toLocaleString('en-AU'), M, 23);
  y = 42;
...
Please find attached my timesheet and expense claim.
...
function startNewSubmission() {
  ...
  if(nextStart){
    ...
    state.employee.fortnightTo = toISO(end);
    document.getElementById('fortnight-to-display').textContent = fmtDate(state.employee.fortnightTo)+' (Sunday)';
    buildDailyTable();
    showToast(`📅 Next fortnight loaded: ${fmtDate(nextStart)}`, 'success');
  }
...
function scheduleReminder(nextStart, nextEnd) {
  ...
  remindFrom.setDate(end.getDate()-2); // Thursday before the Sunday
...
function checkReminder() {
  ...
  try {
    ...
    if(today >= r.remindFrom && today <= r.fortnightEnd){
      const days = Math.round((parseLocalDate(r.fortnightEnd)-new Date())/(1000*60*60*24));
      const txt  = days<=0
...
function saveProgress() {
  collectFormData();
  try {
    localStorage.setItem('fs_draft', JSON.stringify({
      state,
...
function cleanupDraft() { localStorage.removeItem('fs_draft'); }
...
function saveSettings(silent=false) {
  ...
  /* Auto-fill name if the timesheet field is empty */
...
function updateEJSStatus() {
  ...
  if(key && svc && tpl){
    ...
    el.textContent='⚠️ Incomplete — fill all three fields to enable auto-send';
  } else {
...
/** Parse date string as local time (avoids UTC offset shifting the day) */
...
function init() {
  ...
  /* Show reminder if one is due */
  checkReminder();

  /* Empty-state placeholders */
  renderExpenses(); renderMileage(); renderAllowances();

  /* Measure top chrome → set content margin */
  setContentMargin();
  ...
  if(!settings.name && !settings.emailTo){
    setTimeout(()=> showToast('👋 Welcome! Open ⚙️ Settings to set your name', 'success'), 900);
  }

  console.log('VB Built FieldSheet v3 ✅');
}
